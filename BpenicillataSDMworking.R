## woylie species distribution models
## Corey Bradshaw
## January 2020
...
setwd("~/Bettongs/BPenicillata")
DataSpecies <- read.csv("BTBoccur.csv")
head(DataSpecies)
...
data.tmp1 <- subset(DataSpecies, lat < -30)
data.tmp2 <- subset(data.tmp1,  lon < 120)
DataSpecies <- data.tmp2

woyll <- data.frame(as.numeric(DataSpecies$lon),as.numeric(DataSpecies$lat))
colnames(woyll) <- c("longitude","latitude")
plot(woyll$longitude, woyll$latitude, col = 'red', pch=4, cex=0.5)
range(woyll$latitude)
range(woyll$longitude)

presences <- data.frame(rep(1,dim(woyll)[1]))
colnames(presences) <- c("pres")
coordinates(woyll) <- c("longitude","latitude")
proj4string(woyll) <- CRS("+proj=longlat +datum=WGS84")
woy.res <- spTransform(woyll, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
as(woy.res, "SpatialPoints")
woy.proj <- SpatialPointsDataFrame(coords=woy.res, data=presences)

write.csv(woy.proj,"WoyliePresenceOnly_proj.csv")
DataSpecies.proj <- woy.proj
plot(DataSpecies.proj, pch=3)


##+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
##  STEP 3: FORMATTING THE ENVIRONMENTAL DATA
##++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# load the environmental raster layers (could be .img, ArcGIS # rasters or any supported format by the raster package)
# import full rasters

## Australia & states
setwd("~/Bettongs/BPenicillata/States")

library(sf)
wamap <- readOGR(dsn="wa.shp")
plot(wamap)
wa.proj <- spTransform(wamap, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
samap <- readOGR(dsn="sa.shp")
plot(samap)
sa.proj <- spTransform(samap, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
vicmap <- readOGR(dsn="vic.shp")
plot(vicmap)
vic.proj <- spTransform(vicmap, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
nswmap <- readOGR(dsn="nsw.shp")
plot(nswmap)
nsw.proj <- spTransform(nswmap, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
tasmap <- readOGR(dsn="tas.shp")
plot(tasmap)
tas.proj <- spTransform(tasmap, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
qldmap <- readOGR(dsn="qld.shp")
plot(qldmap)
qld.proj <- spTransform(qldmap, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
ntmap <- readOGR(dsn="nt.shp")
plot(ntmap)
nt.proj <- spTransform(ntmap, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))

# Australia
aus0 <- raster::getData('GADM' , country="AUS", level=0) # country outline
aus0.proj <- spTransform(aus0, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))


setwd("~/Bettongs/CSV")

writeOGR(aus0.proj, ".", "aus.lambert", driver="ESRI Shapefile")
aus1 <- raster::getData('GADM' , country="AUS", level=0) # country outline with state outlines
aus1.proj <- spTransform(aus1, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
plot(aus0.proj)

#aus0<- readOGR(dsn="aus.lambert.shp")
#plot(aus0)
#aus0.proj <- spTransform(aus0, CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
#plot(aus0.proj)
...
setwd("~/Bettongs/BPenicillata/ASCII")
rain.asc <- read.asciigrid("m_rainfall.asc", as.image = FALSE, plot.image=T, proj4string = CRS(as.character("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")), colname="RAIN")
rain.rst <- raster(rain.asc, layer=1, values=TRUE)
rain.rst1 <- crop(rain.rst, extent(aus0.proj))
plot(rain.rst1)
points(DataSpecies.proj, pch=3)
ext.dat <- extent(DataSpecies.proj)
...
plot(temp.crop)
temp.rst <- projectRaster(temp.crop, crs=CRS("+proj=lcc +lat_1=-18 +lat_2=-36 +lat_0=0 +lon_0=134 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
plot(temp.rst)  
ext.dat <- extent(DataSpecies.proj)
...
setwd("~/Bettongs/TIFF")
writeRaster(seas.rst, filename="seas.grd", format="raster", overwrite=T)
...
lowres <- ndvi1.rst
# rain.lr <- projectRaster(rain.rst, lowres, method="bilinear")
# temp.lr <- projectRaster(temp.rst, lowres, method="bilinear")
# seas.lr <- projectRaster(seas.rst, lowres, method="bilinear")
# dist2road.lr <- projectRaster(dist2road.rst, lowres, method="bilinear")
# slope.lr <- projectRaster(slope.rst, lowres, method="bilinear")
# elev.lr <- projectRaster(elev.rst, lowres, method="bilinear")
# commchnge.lr <- projectRaster(commchnge.rst, lowres, method="bilinear")
# sand.lr <- projectRaster(sand.rst, lowres, method="bilinear")
# burns.lr <- projectRaster(burns.rst, lowres, method="bilinear")
...
rain.lr <- projectRaster(from=rain.rst, res=lowestres, crs=crs(auslamb.proj), method="bilinear")
temp.lr <- projectRaster(from=temp.rst, res=lowestres, crs=crs(auslamb.proj), method="bilinear")
...
##++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
##  STEP 5: BIOMOD2 SPECIES DISTRIBUTION MODELLING
##++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

setwd("~/Bettongs/BPenicillata/Run")

# studied species
RespName <- 'Bpenicillata'

# the presence/absences data for our species
Resp <- as.numeric(presences$pres)

# the XY coordinates of species data
RespXY <- coordinates(DataSpecies.proj)
plot(RespXY)

BiomodData <- BIOMOD_FormatingData(
                                  resp.var = Resp, 
                                  expl.var = env.stack, 
                                   resp.xy = RespXY, 
                                   resp.name = RespName, 
                                   PA.nb.rep = 3, PA.nb.absences = 5*length(Resp), 
                                   PA.strategy = 'random')
BiomodData
plot(BiomodData)

# 2. Defining Models Options using default options.
BiomodOption <- BIOMOD_ModelingOptions(GLM = gam.control(maxit = 10000))


# 3. Computing the models
# outputs are going to be stored in a folder called 'FirstModeling'
# MARS ,'MAXENT.Phillips' removed
BiomodModelOut <- BIOMOD_Modeling(
  BiomodData,
  models = c('SRE','CTA','RF','FDA','GLM','GBM','GAM','ANN'),
...
models_scores_graph(BiomodModelOut, by = "models", metrics = c("ROC","TSS"),xlim = c(0.6,1), ylim = c(0.4,1))
...
varimpwt.sort
barplot(varimpwt.sort[,2], names.arg=as.character(varimpwt.sort[,1]), ylab="VI")
...
setwd("~/Bettongs/BPenicillata/Run/Bpenicillata/proj_current/individual_projections")

meanbyTSS.dat <- stack(myBiomodEnsembleProj$Bpenicillata_EMwmeanByTSS_mergedAlgo_mergedRun_mergedData)
...
meanbyTSS.dat <- stack("Bpenicillata_EMwmeanByTSS_mergedAlgo_mergedRun_mergedData")

# meanbyTSS.dat <-get_predictions(BiomodEnsembleProj)
plot(meanbyTSS.dat, extent(ext.dat))

plot(wa.proj, add=T)
writeRaster(meanbyTSS.dat, "meanPredWA.grd", format="raster", overwrite=T)

##############################################
# Project to rest of southern Australia
...
setwd("~/Bettongs/BPenicillata/Derived Layers")
...
#plot(BiomodModelProjAus) - this takes forever don't do this
...
# setwd("~/BTB/Results/SDM/woylie/proj_allsaus/individual_projections")
setwd("~/Bettongs/BPenicillata/Run/Bpenicillata/proj_allsaus/individual_projections")

meanbyTSS <- stack("Bpenicillata_EMcaByTSS_mergedAlgo_mergedRun_mergedData")
plot(meanbyTSS, extent(proj.ext.aus))
plot(wa.proj, add=T); plot(sa.proj, add=T); plot(vic.proj, add=T); plot(nsw.proj, add=T); plot(tas.proj, add=T); plot(qld.proj, add=T); plot(nt.proj, add=T)

writeRaster(meanbyTSS, filename="BpenicillatameanPredSAUS.grd", format="raster", overwrite=T)

setwd("~/Bettongs/States")
SAusExtent<-raster("SAusExt.tif")
setwd("~/Bettongs/BPenicillata/Run/Bpenicillata/proj_allsaus/individual_projections")
BpenAus<-raster('BpenicillatameanPredSAUS.grd')
BpenSAus<-crop(BpenAus,SAusExtent)
BpenicillataSAusFinal<-BpenSAus/1000

plot(BpenicillataSAusFinal)
plot(wa.proj, add=T); plot(sa.proj, add=T); plot(vic.proj, add=T); plot(nsw.proj, add=T); plot(tas.proj, add=T); plot(qld.proj, add=T); plot(nt.proj, add=T)
